...
6. Первый подходящий — **скачиваем bytes**, передаём в `imagePersistence.persist(...)` → получаем local URL (dev) или S3 URL (prod)
...
**Фаза 2 — fal.ai fallback** для dishes без Commons hit:
...
- **Скачиваем bytes** из временного fal.ai URL, передаём в `imagePersistence.persist(...)` → получаем local URL
- Результат: `source='fal_ai_generated'`, `license='owned'`, `qualityScore=3` (default)

**Стоимость с Commons optimization**:
- Commons hits: ~1500 images × $0 = **$0**
- fal.ai для остальных: ~3500 × 2 attempts × $0.008 = **~$56**
- **Total ~$56** (экономия $24 vs pure fal.ai подход)

Плюс Commons даёт более "настоящие" фото для популярных блюд — качество выше чем AI generation для случаев где реальная photo существует.

**Время**: ~1 час Commons фаза + ~2-4 часа fal.ai фаза с concurrent batching.

#### 6. `bin/local_file_image_persistence.dart` (infrastructure)

**Не bootstrap скрипт, а инфраструктурная реализация**, которая должна быть
готова **до** запуска `generate_images.dart`.
...
Добавить в `.gitignore`: `web/static/images/curated/` (чтобы 500MB dev
картинок не попали в git).

Подробный дизайн + код — `DATASET_DESIGN.md` секция "Local dev image storage".

**Стоимость**: $0. **Время**: ~1 час написание + test.
...
| Claude translations (×6 languages) | ~$35 |
| fal.ai image generation (~3500 × 2 attempts после Commons filter) | ~$56 |
| **Total bootstrap** | **~$118** |
| | |
| ~~Spoonacular subscription~~ | **НЕ используем** (termination risk) |
| S3 bucket curated-assets (после first deploy) | ~$0.01/мес |
| Per new exotic dish (live fallback) | ~$0.02 |
...
2. **Увеличить `anthropicApiKey` кредиты** — для bootstrap нужно ~$40 на Claude (descriptions + translations). Текущий ключ должен иметь баланс или auto-refill.

3. **Купить fal.ai credits** — минимум $100 на account (покроет bootstrap с запасом + первый месяц live fallbacks).
