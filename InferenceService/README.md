...
- **Deployment**: **Windows native** (без Docker, когда виртуализация выключена) **или** Docker + NVIDIA Container Toolkit
- **Target hardware**: Ryzen 5950x + RTX 3080 Ti 12GB + 64GB RAM (Windows 11)
...
Есть два варианта: **Windows native** (без Docker, нужно когда на машине
выключена виртуализация или нет WSL2) и **Docker** (Linux или Windows с WSL2).
На текущей GPU машине виртуализация выключена → используем **Windows native**.

### Вариант A — Windows native (без Docker)

**Prerequisites** (установить вручную):
1. **NVIDIA GPU driver** (последний) — [nvidia.com/Download](https://www.nvidia.com/Download/index.aspx).
   CUDA Toolkit **не нужен** — PyTorch привозит собственный CUDA runtime.
...
```powershell
# Открыть PowerShell в папке InferenceService
cd C:\path\to\InferenceService

# Разрешить запуск скриптов в текущей сессии (если не разрешено)
Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass

# Запустить установщик: venv + PyTorch CUDA + requirements + .env
.\setup_windows.ps1
```

Что делает `setup_windows.ps1`:
- Проверяет Python 3.10-3.12 и `nvidia-smi`
- Создаёт `venv/` + ставит PyTorch 2.4.1 + CUDA 12.1 wheels (~2.5 GB скачивания)
- Ставит FastAPI/diffusers/transformers (~500 MB)
- Пытается поставить `xformers` (часто падает на Windows — не фатально,
  фоллбэк через `attention_slicing`)
- Генерирует `.env` с random secret
- Проверяет что PyTorch видит GPU (`torch.cuda.is_available()`)

**Сохрани секрет** из .env — он нужен для Serverpod `passwords.yaml`.
...
Первый запрос загружает SDXL в VRAM (~30 сек). Все последующие ~5 сек на картинку.

**Автозапуск при загрузке Windows** (опционально):

Проще всего через Task Scheduler:
1. Открыть Task Scheduler → Create Basic Task
2. Trigger: "When the computer starts"
3. Action: "Start a program"
   - Program: `powershell.exe`
   - Arguments: `-ExecutionPolicy Bypass -File "C:\path\to\InferenceService\run_windows.ps1"`
   - Start in: `C:\path\to\InferenceService`
4. В свойствах задачи → "Run whether user is logged on or not"

Альтернатива — [NSSM](https://nssm.cc/) для запуска как Windows Service.

**Открыть порт 8000 в Windows Firewall**:

```powershell
# Запускать от администратора
New-NetFirewallRule -DisplayName "InferenceService" -Direction Inbound `
    -Protocol TCP -LocalPort 8000 -Action Allow
```

### Вариант B — Docker (требует виртуализацию + WSL2)
...
# Docker + NVIDIA Container Toolkit (Linux пример)
...
На Windows — Docker Desktop с включённой интеграцией WSL2 + GPU passthrough
(требует BIOS-виртуализацию Intel VT-x / AMD-V включённую).
...
**Windows: Docker Desktop не запускается / "virtualization is disabled"**:
- Это значит Intel VT-x / AMD-V выключены в BIOS и/или нет Hyper-V/WSL2
- Включать виртуализацию в BIOS требует ребут + может ломать другой софт
- Проще использовать **Windows native** через `setup_windows.ps1` — полностью
  обходит Docker, тот же pipeline напрямую через Python venv

**Windows: `xformers` install fails**:
- Ожидаемо — prebuilt wheels часто не совпадают с установленной CUDA/Python
- Не фатально: `sdxl_pipeline.py` ловит `ImportError` и откатывается на
  `enable_attention_slicing()` (на ~10% медленнее, чуть больше VRAM — но работает)

**Windows: `.ps1` scripts не запускаются ("execution policy")**:
- В PowerShell запустить: `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass`
- Действует только на текущую сессию, безопаснее чем глобальный Unrestricted

**Windows: порт 8000 недоступен снаружи**:
- Открыть firewall: `New-NetFirewallRule -DisplayName "InferenceService" -Direction Inbound -Protocol TCP -LocalPort 8000 -Action Allow` (от администратора)
- Проверить что на роутере настроен port forwarding на IP GPU машины
- Проверить что uvicorn слушает `0.0.0.0`, не `127.0.0.1` (по умолчанию в `run_windows.ps1` — `0.0.0.0`)
